...
record Typechecker (v : Nat) (a : Mode -> Set) : Set where
  field
    var     : Nat -> a SYN
    lam     : (Nat -> a CHK) -> a CHK
    ...
    have    : {mode : Mode} -> Nat -> (Type -> a mode) -> a mode -- lookup the type of a variable
    goalIs  : {mode : Mode} -> (Type -> a mode) -> a mode        -- looking up the type of the goal
    failure : {mode : Mode} -> a mode

open Typechecker {{...}}

data TCTerm (v : Nat) (a : Set) : Set where
   Var     : Nat -> TCTerm v a
   ...
   Have    : Nat -> Type -> TCTerm v a -> TCTerm v a
   GoalIs  : Type -> TCTerm v a -> TCTerm v a
   Failure : TCTerm v a

_>>=_ : {v : Nat} -> {a b : Set} -> TCTerm v a -> (a -> TCTerm v b) -> TCTerm v b
Var i        >>= _ = Var i
...
assumption : {v : Nat} -> {a : Set} -> Nat -> TCTerm v a
assumption = Var
